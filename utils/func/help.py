...
class Help:
    ...
    def _HelpEmbedBasic(self) -> discord.Embed:
        embed = discord.Embed(title=":regional_indicator_q: | 指令說明 | 基本指令", description="若遇到錯誤可以先閱讀訊息所提示的方法來排錯喔", colour=0xF2F3EE)
        embed.add_field(name="/help", value="你目前就正在用這個喔，輸入會列出指令說明", inline=False)
        embed.add_field(name="/join", value="將機器人加入到您目前所在的語音頻道\n**【！】若遇到錯誤 JOINFAIL**\n*可能是您沒有加入到任一語音頻道中，或是機器人無權限加入該頻道導致*", inline=False)
        embed.add_field(name="/leave", value="使機器人離開其所在的語音頻道\n**【！】若遇到錯誤 LEAVEFAIL**\n*可能是機器人並沒有加入到任一語音頻道中導致*", inline=False)
        return embed
    ...
    def _HelpEmbedPlayback(self, page: int) -> discord.Embed:
        ...
        if page == 1:
            ...
            embed.add_field(name="/resume", value="繼續播放已暫停的歌曲\n**【！】若遇到錯誤 RESUMEFAIL**\n*可能目前沒在播放歌曲，或目前歌曲未被暫停導致*", inline=False)
            embed.add_field(name="/skip", value="跳過目前的歌曲\n**【！】若遇到錯誤 SKIPFAIL**\n*可能目前沒在播放歌曲導致*", inline=False)
        elif page == 2:
            embed.add_field(name="/stop", value="停止歌曲並清除所有待播清單中的歌曲\n**【！】若遇到錯誤 STOPFAIL**\n*可能因目前沒在播放歌曲導致*", inline=False)
            embed.add_field(name="/seek [秒/時間戳]", value="快轉至指定時間\n*參數支援: 秒 (如: 2) / 時間戳 (如: 0:02)*\n**【！】若遇到錯誤 SEEKFAIL**\n*可能因輸入的跳轉時間無效(格式錯誤)或目前沒在播放歌曲導致*", inline=False)
            embed.add_field(name="/restart", value="重新播放目前歌曲\n**【！】若遇到錯誤 REPLAYFAIL**\n*可能因目前沒在播放歌曲導致*", inline=False)
            embed.add_field(name="/loop [次數]", value="切換單曲循環開關 (輸入次數則可指定單曲播放幾次)\n**【！】若遇到錯誤 LOOPFAIL_SIG**\n*可能因指定的重複次數無效 (小於0一類的) 或目前沒在播放歌曲導致*", inline=False)
            embed.add_field(name="/wholeloop", value="切換全待播清單循環開關", inline=False)
        return embed 
    
    def _HelpEmbedQueue(self) -> discord.Embed:
        embed = discord.Embed(title=":regional_indicator_q: | 指令說明 | 待播清單相關指令", description="若遇到錯誤可以先閱讀訊息所提示的方法來排錯喔", colour=0xF2F3EE)
        embed.add_field(name="/queue", value="顯示待播歌曲列表", inline=False)
        embed.add_field(name="/shuffle", value="隨機排列待播歌曲列表\n**【！】若遇到錯誤 SHUFFLEFAIL**\n*可能是因目前待播清單無歌曲導致*", inline=False)
        embed.add_field(name="/remove [順位數]", value="移除指定待播歌曲 (順位數可以依 /queue 顯示的代號輸入)\n**【注意】建議歌曲是不能移除的，嘗試移除會導致錯誤 REMOVEFAIL**\n**【！】若遇到錯誤 REMOVEFAIL**\n*可能是因輸入的順位數無效 (根本沒這首歌) 或目前待播清單無歌曲導致*", inline=False)
        embed.add_field(name="/swap [順位數1] [順位數2]", value="交換指定待播歌曲順序 ([順位數1] <-> [順位數2])\n**【注意】建議歌曲是不能交換的，嘗試交換會導致錯誤 SWAPFAIL**\n**【！】若遇到錯誤 SWAPFAIL**\n*可能是因輸入的順位數無效 (根本沒這首歌) 或目前待播清單無歌曲導致*", inline=False)
        embed.add_field(name="/move [原順位數] [目標順位數]", value="移動指定待播歌曲至指定順序 ([原順位數] -> [目標順位數])\n**【注意】建議歌曲是不能移動的，嘗試移動會導致錯誤 MOVEFAIL**\n**【！】若遇到錯誤 MOVEFAIL**\n*可能是因輸入的順位數無效 (根本沒這首歌) 或目前待播清單無歌曲導致*", inline=False)
        return embed
