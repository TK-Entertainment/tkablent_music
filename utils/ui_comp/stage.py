...
class Stage:
    ...
    async def CreateStageInstance(
        self, interaction: discord.Interaction, guild_id: int
    ) -> None:
        if (
            isinstance(interaction.user.voice.channel.instance, discord.StageInstance)
            or self.auto_stage_available(guild_id) == False
        ):
            return
    ...
    async def EndStage(self, guild_id: int) -> None:
        if not self.auto_stage_available(guild_id):
            return
        if not isinstance(
            self.bot.get_guild(guild_id).voice_client.channel.instance,
            discord.StageInstance,
        ):
            return
        channel: discord.StageChannel = self.bot.get_guild(
            guild_id
    ...
    async def _UpdateStageTopic(self, guild_id: int, mode: str = "update") -> None:
        ...
        if self.auto_stage_available(guild_id) == False or isinstance(
            self.bot.get_guild(guild_id).voice_client.channel, discord.VoiceChannel
        ):
            return
        instance: discord.StageInstance = self.bot.get_guild(
            guild_id
        ).voice_client.channel.instance
        if (
            (instance.topic != "🕓 目前無歌曲播放 | 等待指令")
            ...
            if mode == "done":
                ...
                await instance.edit(
                    topic="{}{} {}{}".format(
                        ...
                        playlist[0].title[:40]
                        if len(playlist[0].title) >= 40
                        else playlist[0].title,
