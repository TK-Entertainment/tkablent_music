...
class UI:
    ...
    def __SongInfo__(self, color: str=None, playlist: Playlist=None, index: int=0, mute: bool=False):
        ...
        embed = disnake.Embed(title=self.playlist[index].title, url=self.playlist[index].watch_url, colour=colorcode)
        ...
        if self.is_stream: 
            ...
            if mute: embed.set_author(name=f"這首歌由 {self.playlist[index].requester.name}#{self.playlist[index].requester.tag} 點歌 | 🔴 直播 | 🔇 靜音", icon_url=self.playlist[index].requester.display_avatar)
            else: embed.set_author(name=f"這首歌由 {self.playlist[index].requester.name}#{self.playlist[index].requester.tag} 點歌 | 🔴 直播", icon_url=self.playlist[index].requester.display_avatar)
        else: 
            embed.add_field(name="歌曲時長", value=sec_to_hms(self.playlist[index].length, "zh"), inline=True)
            # The mute notice
            if mute: embed.set_author(name=f"這首歌由 {self.playlist[index].requester.name}#{self.playlist[index].requester.tag} 點歌 | 🔇 靜音", icon_url=self.playlist[index].requester.display_avatar)
            else: embed.set_author(name=f"這首歌由 {self.playlist[index].requester.name}#{self.playlist[index].requester.tag} 點歌", icon_url=self.playlist[index].requester.display_avatar)
        if len(playlist) > 1:
            queuelist: str = ""
        ...
        embed.set_thumbnail(url=self.playlist[index].thumbnail_url)
    ...
    async def StartPlaying(self, ctx: commands.Context, playlist: Playlist, ismute: bool):
        ...
        playinfo = await ctx.send(f'''
            ...
            *輸入 **{self.__bot__.command_prefix}pause** 以暫停播放*
            ''', embed=self.__SongInfo__(self.__embed_opt__, sec_to_hms, self.__bot__.command_prefix, playlist=playlist, mute=ismute))
