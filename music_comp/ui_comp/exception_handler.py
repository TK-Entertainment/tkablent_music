...
class ExceptionHandler:
    def __init__(self, info_generator):
        ...
        self.errorcode_to_msg = {
            ...
            "PAUSEFAIL": [
                "無法暫停音樂，請確認目前有歌曲正在播放，或是當前歌曲已處於暫停狀態，亦或是候播清單是否為空",
                "pause",
                "來暫停音樂",
            ],
            ...
            "LOOPFAIL_SIG": [
                "無法啟動重複播放功能，請確認您輸入的重複次數有效，或是機器人有在播放歌曲",
                f"loop / {self.bot.command_prefix}loop [次數]",
                "來控制重複播放功能",
            ],
    ...
    async def _MusicExceptionHandler(
        self, message: discord.Interaction, exception=None, url=None
    ):
        part_content = f"""
        ...
        *此錯誤不會影響到播放，僅為提醒訊息*"""
        ...
        await self._BugReportingMsg(
            ...
            content,
            done_content,
            errorcode="SEARCH_OR_PLAYING_FAILED",
            exception=exception,
            video_url=url,
        )

    async def _CommonExceptionHandler(
        self, message: discord.Interaction, errorcode: str, exception=None
    ):
        done_content = f"""
        ...
        await self._BugReportingMsg(
            message, content, done_content, errorcode, exception
        )

    async def _BugReportingMsg(
        self,
        message: Union[discord.Interaction, discord.TextChannel],
        content,
        done_content,
        errorcode,
        exception=None,
        ...
        if errorcode == "SEARCH_FAILED":
            embed = self.info_generator._SongInfo(
                guild_id=message.guild.id, color_code="red"
            )
            if isinstance(message, discord.Interaction) and message.is_response():
                ...
                msg = await message.send(
                    content, embed=embed, view=groupbutton, ephemeral=True
                )
        else:
            if isinstance(message, discord.Interaction):
                if message.response.is_done():
                    msg = await message.edit_original_response(
                        content=content, view=groupbutton
                    )
                else:
                    msg = await message.response.send_message(
                        content, view=groupbutton, ephemeral=True
                    )
            else:
